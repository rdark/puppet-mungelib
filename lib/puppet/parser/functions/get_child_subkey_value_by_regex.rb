#
# get_child_subkey_value_by_regex.rb
#
# === Author
#
...
module Puppet::Parser::Functions
  newfunction(:get_child_subkey_value_by_regex, :type => :rvalue, :doc => <<-EOS
...
EOS
    ...
    if arguments.size != 3 then
      raise(ArgumentError, "get_child_subkey_value_by_regex(): Wrong number of arguments "+
          ...
          if parent.match(r)
            child_regex.each do |c|
              if c.match(child)
                ...
                dest_array << src_hash[parent][child]
                # break so we don't get duplicates if overlapping child regexes match
                break
              end
            end
          end
