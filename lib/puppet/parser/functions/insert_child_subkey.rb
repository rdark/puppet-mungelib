...
module Puppet::Parser::Functions
  newfunction(:insert_child_subkey, :type => :rvalue, :doc => <<-EOS
...
EOS
    ...
    if (arguments.size != 3) then
      raise(Puppet::ParseError, "insert_child_subkey(): Wrong number of arguments "+
    ...
    src_hash.keys.each do |parent|
        ...
        if parent =~ /#{r}/
          ...
          unless existing_children
            src_hash[parent].keys.each do |child|
              # insert the key into the hash unless it appears in insert hash
              # (so RHS keys overwrite LHS)
              dest_hash[parent][child] = src_hash[parent][child] unless insert_hash.has_key?(child)
            end
      ...
      # if no regex matches then we need to add the parent hash the hash over
